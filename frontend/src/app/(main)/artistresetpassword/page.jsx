...
const artistresetpassword = () => {
    ...
    const checkMailExists = async () => {
        const res = await fetch(
            ` ${process.env.NEXT_PUBLIC_API_URL}/artist/getbymail/${emailRef.current.value}`
        );
        console.log(res.status);
        const data = await res.json();
        // console.log(data);
        setVerifiedArtist(data);
    ...
    const resetForm = useFormik({
        ...
        onSubmit: async (values) => {
            console.log(values);
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/artist/update/${verifiedArtist._id}`,
                {
                    method: "PUT",
                    body: JSON.stringify({
